...
# 原信号频谱
sig1_fft_amp = fftshift(np.abs(fft(sig1)) * Ts)  # 双边幅度谱,范围为正负sample_freq/2
# 载波信号频谱
carrier1_fft_amp = fftshift(np.abs(fft(carrier1)) * Ts)  # 双边幅度谱,范围为正负sample_freq/2
# 调制信号频谱
mod_sig1_fft_amp = fftshift(np.abs(fft(mod_sig1)) * Ts)  # 双边幅度谱,范围为正负sample_freq/2
...
# 原信号频谱
sig2_fft_amp = fftshift(np.abs(fft(sig2)) * Ts)  # 双边幅度谱,范围为正负sample_freq/2
# 载波信号频谱
carrier2_fft_amp = fftshift(np.abs(fft(carrier2)) * Ts)  # 双边幅度谱,范围为正负sample_freq/2
# 调制信号频谱
mod_sig2_fft_amp = fftshift(np.abs(fft(mod_sig2)) * Ts)  # 双边幅度谱,范围为正负sample_freq/2
...
mix_sig_fft = fftshift(fft(mix_sig)) * Ts  # 双边幅度谱,范围为正负sample_freq/2
...
# 分离后信号的时域形式
BPF_sig1 = (ifft(ifftshift(BPF_sig1_fft / Ts)))
BPF_sig2 = (ifft(ifftshift(BPF_sig2_fft / Ts)))
...
# 第一路信号
sd.play(rt1, fs, blocking=True)
# 第二路信号
sd.play(rt2, fs, blocking=True)
